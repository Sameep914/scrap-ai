...
INPUTS = ROOT / "inputs"
OUTPUTS = ROOT / "outputs" / "v3_overlap_adjusted"
...
def load_turkey_nearby() -> tuple[pd.DataFrame, dict]:
    path = INPUTS / "turkey_scrap_overlap_adjusted.csv"
    frame = pd.read_csv(path)
    frame["date"] = pd.to_datetime(frame["date"], errors="coerce")
    frame = frame[frame["date"].notna()].copy()
    for column in [
        "Open", "High", "Low", "Latest", "Change", "Volume", "Open Int",
        "overlap_adjusted_log_return", "overlap_adjusted_index",
        "turkey_mom5", "turkey_mom10", "turkey_rv10",
        "turkey_zero_range20", "turkey_zero_oi20",
    ]:
        frame[column] = as_float(frame[column])
    frame = frame.sort_values("date").drop_duplicates("date").reset_index(drop=True)
    frame["roll"] = frame["roll"].astype(str).str.lower().eq("true")
    frame["roll_adjusted_return"] = frame["overlap_adjusted_log_return"]
    frame["roll_adjusted_index"] = frame["overlap_adjusted_index"]
    ...
    audit = {
        ...
        "duplicate_dates_removed": int(pd.read_csv(path)["date"].duplicated().sum()),
        ...
        "roll_adjusted_return_missing": int(
            frame.loc[roll_rows, "roll_adjusted_return"].isna().sum()
        ),
        "roll_change_vs_raw_diff_max_abs": float(
            (raw_difference.loc[roll_rows] - frame.loc[roll_rows, "Change"]).abs().max()
        ),
        "roll_return_formula": (
            "selected incoming fixed contract's own close-to-close return on switches; "
            "this removes the prior shared-close contract basis and preserves the market move"
        ),
    }
    return frame, audit
...
def main() -> None:
    ...
    panel.to_csv(OUTPUTS / "master_panel_v3.csv", index=False)
    ...
    predictions.to_csv(OUTPUTS / "oos_predictions_v3.csv", index=False)
    summaries.to_csv(OUTPUTS / "backtest_summary_v3.csv", index=False)
    phases.to_csv(OUTPUTS / "phase_offset_stability_v3.csv", index=False)
    pairwise.to_csv(OUTPUTS / "same_origin_pairwise_v3.csv", index=False)
    adjustment.to_csv(OUTPUTS / "multiple_testing_v3.csv", index=False)
    calls.to_csv(OUTPUTS / "no_call_summary_v3.csv", index=False)
    live.to_csv(OUTPUTS / "live_forecasts_v3.csv", index=False)
    decisions.to_csv(OUTPUTS / "horizon_decisions_v3.csv", index=False)
    ...
    metadata = {
        "study": "8ANI leakage-safe forecast study v3, overlap-adjusted Turkey contracts",
        "seed": SEED,
        ...
        "input_hashes_sha256": {
            ...
            "turkey_scrap_overlap_adjusted.csv": sha256(INPUTS / "turkey_scrap_overlap_adjusted.csv"),
    ...
    (OUTPUTS / "study_metadata_v3.json").write_text(json.dumps(metadata, indent=2, default=str), encoding="utf-8")
    (OUTPUTS / "multiple_testing_metadata_v3.json").write_text(
